...
public class linkedList { // class of the file
    ...
    public void addToFront(int value) // here we are creating a new function/ method to
    {
        ...
        newNode.next = head; // next node/ pointer of this new node is pointed towards head pointer which was initially alloted to either null or the the first element
        head = newNode; // since we are adding the node at the beginning the head pointer is now updated to the address of the new node.
        if (newNode.next == null) { // this is in a case if there are no elements in the linkedList initially. so this will  allot the tail also to the new node.
            tail = newNode;
        }
    }
    ...
    public boolean contains(int value) { // this method basically checks if a particular value exists in the linked list or not. Hence the return type is boolean
        ...
        while (current != null) { // traversing the current node til it hits an empty node(null)
            if (current.value == value) { // if the value of the current node is equal to the value that is asked, then it returns true. which means that the value exists in the list
                return true;
            }
    ...
    public int getByIndex(int index) { // In this method we will try to get the value of the element of a node with the help of its index. We give the input as the index of the node
        if (index < 0) { // if the input is negative then, it will throw an inbulit exception.
            ...
            for (int i = 0; i < index; i++) {
                if (current == null || current.next == null) {
                    throw new IndexOutOfBoundsException(); // if the current node traverses til the end and doesn't get the index mentioned. then it will throw an exception. For example, if we search index 7 in a 4 data linked list.
                }
    ...
    public void removeFromBack() { // we have to remove a node from the rear end of this linked list.
        if (head == null) { // empty list
            ...
            Node current = head; // we create a new node for traversal and we initailise that to head. we will now travel till the next node is tail. we will allocate tail to the previous node and update the next node to null. thus, removing the last element.
