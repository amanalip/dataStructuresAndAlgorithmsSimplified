...
value exists. To solve this issue we will create an array of linkedList. Also known as Ragged Array(A two dimensional array with variable 
row lengths).There is one flaw in this program. That is, what if a particular index has more and more values coming. Then that particular
index will grow beyond measure. If an index grows beyond measure, then searching a value in that increases in its time complexity and we
will lose the initial benefits we had of creating a hash set.The solution to this problem is to resize the length of the hashset 
automatically to make the whole hashset. Now we can reduce the length of each index in the hashset.Please find the file 
"HashSetsWithSelfResizing.java" in the current directory. */
public class HashSetWithSeparateChaining {
    ...
    public boolean add(String value) { // in this method we add a value to the hashset.
        if (!contains(value)) { //sets don't allow repetition of values.
            ...
            we use it to get the appropriate linked list. Now we assign this linked list's memory location to a dummy variable bucket.*/
    ...
    public boolean contains(String value) { // this methods checks if the set already contains a value or not.
        ...
        LinkedList < String > bucket = buckets[index]; /* reaching out to the particular linked list and 
        then assigning that value to a dummy variable bucket.  */
