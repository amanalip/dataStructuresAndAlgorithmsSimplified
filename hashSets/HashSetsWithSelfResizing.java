public class HashSetsWithSelfResizing {
    ...
    private double loadFactor; // loadfactor should be greater than currentsize/length of the hash set. if this inequality changes, we increase the length of the hashset so that we can again increase the efficiency of searching.
    ...
    private void reinsertAll() {
        LinkedList < String > oldBuckets[] = buckets; // here we will store the value of the hashset into another hashset for reference purposes
        ...
        for (LinkedList < String > bucket: oldBuckets) { // for each linkedlist in the old bucket, each element in that linked list is selected.
